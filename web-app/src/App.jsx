...
function AppInner() {
  ...
  const adjustQuantity = async (itemId, amount, targetQty) => {
    ...
    try {
      ...
      await logChange(itemId, newQty, delta > 0 ? `+${delta}` : `${delta}`);
    } catch (err) { console.error(err); }
  };
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[var(--bg)] flex">
        ...
        <div className="hidden lg:flex flex-col w-[480px] shrink-0 relative overflow-hidden p-12 bg-gradient-to-br from-indigo-950 via-violet-950 to-slate-950">
          {/* Background glow orbs */}
          <div className="absolute top-1/4 -left-16 w-80 h-80 bg-indigo-600/25 rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="flex flex-col gap-4">
            <FeatureBullet
              ...
              icon={<svg className="w-4 h-4 text-indigo-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>}
            />
            <FeatureBullet
              label={t('authFeature2')}
              icon={<svg className="w-4 h-4 text-indigo-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>}
            />
            <FeatureBullet
              label={t('authFeature3')}
              icon={<svg className="w-4 h-4 text-indigo-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg>}
            />
          </div>
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)] flex flex-col">
      {/* Header */}
      <header className="sticky top-0 z-40 border-b border-[var(--border)] backdrop-blur-xl" style={{ background: 'var(--header)' }}>
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          {/* Brand */}
          <div className="flex items-center gap-2.5 shrink-0">
            <div className="w-8 h-8 bg-gradient-to-tr from-indigo-500 to-violet-600 rounded-lg flex items-center justify-center shadow shadow-indigo-500/25">
          ...
          {/* Nav */}
          <nav className="flex items-center gap-1 bg-[var(--input-bg)] border border-[var(--input-border)] rounded-xl p-1">
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            <div className="hidden sm:flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-gradient-to-tr from-indigo-500 to-violet-500 flex items-center justify-center text-white text-[10px] font-bold uppercase">
                {displayName.charAt(0)}
              </div>
              <span className="text-xs font-medium text-[var(--text-2)] max-w-[100px] truncate">{displayName}</span>
            </div>
            <button
